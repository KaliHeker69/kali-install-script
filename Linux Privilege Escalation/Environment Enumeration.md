...
```shell-session
kaliheker69@htb[/htb]$ cat /etc/os-release
...
```shell-session
kaliheker69@htb[/htb]$ cat /etc/shells
...
```shell-session
kaliheker69@htb[/htb]$ cat /etc/fstab
...
```shell-session
kaliheker69@htb[/htb]$ cat /etc/passwd
...
```shell-session
kaliheker69@htb[/htb]$ cat /etc/passwd | cut -f1 -d:
...
```shell-session
kaliheker69@htb[/htb]$ grep "*sh$" /etc/passwd
...
```shell-session
kaliheker69@htb[/htb]$ cat /etc/group
...
```shell-session
kaliheker69@htb[/htb]$ getent group sudo
...
```shell-session
kaliheker69@htb[/htb]$ cat /etc/fstab | grep -v "#" | column -t
...
```shell-session
kaliheker69@htb[/htb]$ find / -type f -name ".*" -exec ls -l {} \; 2>/dev/null | grep htb-student
...
```shell-session
kaliheker69@htb[/htb]$ find / -type d -name ".*" -ls 2>/dev/null
...
```shell-session
kaliheker69@htb[/htb]$ ls -l /tmp /var/tmp /dev/shm
